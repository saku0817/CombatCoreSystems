...
CCS武器は`weapons.yml`の`category`（または`type`）でMELEE／RANGED／UNCATEGORIZEDを指定します。近接・遠距離はホットバーとオフハンドに同じ種別を1本まで保持でき、余分な武器はメインインベントリ、空きがなければ地面へ移動します。未指定は複数所持可能です。ATKと属性ボーナスは両手の有効武器だけを加算します。強化GUIでは素材使用数、強化前後、上昇幅を確認できます。
...
最新の操作・設定キー・変更理由と前後は [v1.4.0ガイド](docs/v1.4.0.md) を参照してください。スキルは手持ち武器のドロップ操作（実際には落とさない）、必殺技はしゃがみ＋攻撃（通常攻撃をキャンセル、コマンド併用可）。同梱の`guiding_star`は天賦・HP消費・バフ・複合ダメージ・落日の動作例です。既存設定を更新する際は、武器定義とbuffs.ymlの参照先をセットで追加してください。
